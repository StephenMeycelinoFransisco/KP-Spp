                ...
                <div class="d-flex align-items-end row">
                    <div class="col-sm-7">
                        <div class="card-body">
                            ...
                            <p class="mb-4">
                                Kamu mendapatkan <span class="fw-bold">{{ auth()->user()->unreadNotifications->count() }}</span> Notifikasi yang belum kamu lihat. Klik tombol di bawah
untuk melihat informasi pembayaran. </p>
                            ...
                            <div class="card-title d-flex align-items-start justify-content-between">
                                ...
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-end" aria-labelledby="cardOpt6">
                                        <a class="dropdown-item" href="{{ route('pembayaran.index') }}">Lihat Lengkapnya</a>
                                    </div>
                                </div>
                            </div>
                            <span>Total Sudah Bayar</span>
                            <h3 class="card-title text-nowrap mb-1">{{ $totalSiswaSudahBayar }} Siswa</h3>
                            <small class="text-success fw-semibold"><i class="bx bx-up-arrow-alt"></i>{{ formatRupiah($totalPembayaran) }}</small>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="card h-100">
                <div class="card-header d-flex align-items-center justify-content-between pb-0">
                    <div class="card-title mb-0">
                        <h5 class="m-0 me-2">Tagihan {{ $tahun }}</h5>
        ...
        <div class="col-md-6 col-lg-4 order-2 mb-4">
            <div class="card h-100">
                <div class="card-header d-flex align-items-center justify-content-between">
                    <h5 class="card-title m-0 me-2">Pembayaran yang belum dikonfirmasi</h5>
                    <div class="dropdown">
                    ...
                    <ul class="p-0 m-0">
                        @foreach ( $dataPembayaranBelumKonfirmasi->take(10) as $item)
                            <li class="d-flex mb-4 pb-1">
                                <div class="avatar flex-shrink-0 me-3">
                                    <span class="avatar-initial rounded bg-label-primary">
                                        <img src="{{ \Storage::url($item->tagihan->siswa->foto) }}"/>
                                    </span>
                                </div>
                                <div class="d-flex w-100 flex-wrap align-items-center justify-content-between gap-2">
                                    <div class="me-2">
                                        <small class="text-muted d-block mb-1">
                                            {{-- {{ $item->tagihan->tanggal_tagihan->translatedFormat('F Y') }} --}}
                                            {{ $item->tanggal_bayar->diffForHumans() }}
                                        </small>
                    ...
                    <ul class="p-0 m-0">
                        @foreach ($tagihanBelumBayar->take(10) as $item)
                            <li class="d-flex mb-4 pb-1">
                                <div class="avatar flex-shrink-0 me-3">
                                    <span class="avatar-initial rounded bg-label-primary">
                                        <img src="{{ \Storage::url($item->siswa->foto) }}"/>
                                    </span>
                                </div>
                                <div class="d-flex w-100 flex-wrap align-items-center justify-content-between gap-2">
                                    <div class="me-2">
